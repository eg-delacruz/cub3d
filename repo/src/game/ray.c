...
/*   Updated: 2025/08/22 00:21:48 by jtivan-r         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_ray(t_ray *ray, t_player *p, int x)
{
	...
	cam_x = 2 * x / (double)SCREEN_W - 1;
	set_dvector(&ray->dir, p->curr_dir[X] + p->plane[X] * cam_x, \
p->curr_dir[Y] + p->plane[Y] * cam_x);
	get_delta_dist(ray);
...
void	find_wall(t_ray *ray, t_game *game, t_ivector map_pos)
{
	while (ray->hit == 0)
	{
		...
		if (map_pos[X] < 0 || map_pos[X] >= game->map_cols || \
map_pos[Y] < 0 || map_pos[Y] >= game->map_rows)
		{
			ray->hit = 1;
			break ;
		}
